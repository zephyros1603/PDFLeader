...
export default function Toolbar({
  ...
  const handleImageInsert = (e) => {
    ...
    if (!file) return
    // For now, just acknowledge image support
    alert('Image insert: select a spot on the PDF to place the image. (Feature uses signature placement mechanism)')
    e.target.value = ''
  }
  ...
  return (
    <div className="toolbar">
      ...
      {/* Quick colors */}
      <div className="toolbar-group" style={{ flexDirection: 'row', flexWrap: 'wrap', gap: '3px', padding: '4px 6px' }}>
        {['#000000', '#EF4444', '#F59E0B', '#10B981', '#3B82F6', '#8B5CF6', '#FFFFFF'].map(color => (
          <button
            ...
            style={{
              ...
              height: 16,
              borderRadius: '50%',
              border: activeColor === color ? '2px solid #5B4CF5' : '1px solid rgba(0,0,0,0.15)',
              background: color,
              cursor: 'pointer',
              padding: 0,
              flexShrink: 0
